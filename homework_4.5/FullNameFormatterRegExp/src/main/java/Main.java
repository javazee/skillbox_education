  ...
  public static void main(String[] args) {
    ...
    while (true) {
      ...
      if (Pattern.matches(regex, input)) {
        if (family.find()){
          System.out.println("Фамилия: " + input.substring(family.start(), family.end()-1));
        }
        if (family.find()){
          System.out.println("Имя: " + input.substring(family.start(), family.end()-1));
        }
        if (family.find()){
          System.out.println("Отчество: " + input.substring(family.start(), family.end()));
        }
      } else {
